  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const { data } = await api.get(
        `/workspaces/${workspaceId}/clientes?${params.toString()}`,
      );
...
export const useCliente = (workspaceId: string, clienteId: string) => {
  return useQuery({
    ...
    queryFn: async () => {
      const { data } = await api.get(
        `/workspaces/${workspaceId}/clientes/${clienteId}`,
      );
      return data as ClienteData;
    },
...
export const useClientesAnalytics = (workspaceId: string) => {
  return useQuery({
    ...
    queryFn: async () => {
      const { data } = await api.get(
        `/workspaces/${workspaceId}/clientes/analytics`,
      );
  ...
  return useMutation({
    mutationFn: async (dto: CreateClienteDTO) => {
      const { data } = await api.post(
        `/workspaces/${workspaceId}/clientes`,
        dto,
      );
      return data as ClienteData;
    },
...
export const useActivateCliente = (workspaceId: string, clienteId: string) => {
  ...
  return useMutation({
    mutationFn: async () => {
      const { data } = await api.patch(
        `/workspaces/${workspaceId}/clientes/${clienteId}/activate`,
      );
      return data as ClienteData;
    },
    onSuccess: () => {
...
export const useDeleteCliente = (workspaceId: string, clienteId: string) => {
  const queryClient = useQueryClient();
